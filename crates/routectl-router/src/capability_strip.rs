...
//!
//! [`StripKind::AssistantReasoning`] has no `strip_plan` row and is applied
//! nowhere in this module: which replayed reasoning artifacts are portable
//! depends on the TARGET LANE's replay scheme, not on the capability key
//! alone, so that transform lives in the router's replay-repair module behind
//! the wrapper that also re-stamps the calibration estimate.
//!
...
pub enum StripKind {
    ...
    /// are portable depends on the target lane's replay scheme -- so it lives
    /// in the router's replay-repair module rather than in the key-only
    /// [`strip_plan`] table, and the interceptor skips it.
    AssistantReasoning,
}
        ...
        let mut req = ChatRequest {
            messages: Arc::from(vec![Message {
                ...
                reasoning_details: vec![ReasoningDetail {
                    ...
                    id: Some("rs_foreign".to_string()),
                    format: Some(BEDROCK_MANTLE.to_string()),
                    index: None,
