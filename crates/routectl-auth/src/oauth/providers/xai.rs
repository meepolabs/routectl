...
/// retry. Any other refresh failure maps to `TokenEndpoint`. Neither path
/// echoes the upstream body (the refresh body can carry the long-lived
...
///   to `prior_refresh` when xAI omits (or empties) it. On exchange
...
mod tests {
    ...
    fn parse_error_never_leaks_token_from_body() {
        let body = r#"{"access_token":"placeholder","expires_in":"sk-xai-REALTOKENLEAK99"}"#;
        ...
        assert!(!msg.contains("sk-xai"), "leaked token prefix: {msg}");
