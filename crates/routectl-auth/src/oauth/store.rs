...
//! disk reads/writes through async-aware locks. a prior change ships read-only
//! resolution + login writeback; a prior change adds the refresh single-flight
...
/// triggers refresh in a prior change. a prior change reports an error and tells the operator
/// to re-login.
...
impl SecretStore for OAuthStore {
    async fn get(&self, secret_ref: &SecretRef) -> Result<String> {
        ...
        if rec.near_expiry(REFRESH_LEAD_SECS, unix_now()) {
            // a prior change: no refresh yet. Tell the operator clearly.
            // a prior change replaces this branch with refresh + return new token.
            tracing::warn!(
                provider = %provider,
    ...
    async fn on_auth_failure(&self, secret_ref: &SecretRef) -> Result<()> {
        ...
        // instead of generic guidance. a prior change has no refresh path; a prior change
