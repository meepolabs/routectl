        ...
        for (k, v) in &source {
            ...
            if self.cfg.auth_kind == AuthKind::ChatgptOauth && is_codex_fingerprint_header(k) {
                tracing::debug!(
                    provider = %self.cfg.id,
                    header = %k,
                    "dropping codex client header from header_extras on chatgpt-oauth path"
                );
                continue;
            }
...
/// upstream validates on every request.
...
mod fingerprint_tests {
    ...
    /// share a name with a codex client header (e.g., `version`)
