...
//!
//! Three auth surfaces (only one wired in CG.A):
//!
...
//!
//! Response / SSE / Provider impl wiring all land in CG.B + CG.C;
...
//! locked behind the live smoke gate in CG.C.
...
impl Provider for OpenAiResponsesProvider {
    ...
    fn normalize_response(&self, _raw: Value) -> Result<ChatResponse> {
        // CG.B lands the response side. Today's invocation surfaces a
        ...
        Err(Error::normalize_response(
            &self.cfg.id,
            "openai-responses response decoding not yet implemented (CG.B)",
        ))
    }
    ...
    async fn complete(&self, req: ChatRequest) -> Result<ChatResponse> {
        ...
        // CG.B will replace this stub with a full upstream call +
        ...
        Err(Error::normalize_response(
            &self.cfg.id,
            "openai-responses complete() not yet implemented (CG.B)",
        ))
    }

    #[tracing::instrument(skip_all, fields(provider = %self.cfg.id, model = %sanitize_for_log(&req.model)))]
    async fn stream(&self, req: ChatRequest) -> Result<BoxStream<'static, Result<ChatChunk>>> {
        // CG.B owns the SSE/eventstream decoder. Emit a NotImplemented-
        // shape error rather than panic so an early caller sees a
        // clean failure path.
        let _ = self.normalize_request(&req)?;
        Err(Error::Streaming(format!(
            "openai-responses provider `{}`: stream() not yet implemented (CG.B)",
            self.cfg.id
        )))
    }
}
