...
//!   ```
//!   The codex CLI's upstream inspects every routectl-emitted
//!   request claiming `originator: codex_cli_rs` and invalidates
//!   sessions whose HTTP client identity drifts from a real codex
//!   install. Mirrors:
...
/// The upstream on chatgpt.com fingerprints both the literal
/// `originator` value AND the UA shape; emitting `routectl/<ver>
/// codex-cli` here is enough drift to trigger re-authentication auth on the next
/// codex login.
...
fn apply_chatgpt_oauth(
    ...
    // thread-id into x-client-request-id; the upstream expects the
    // pair to match.
...
mod tests {
    ...
    fn window_id_is_process_global_across_distinct_providers() {
        ...
        // the chatgpt.com client parity contract.
