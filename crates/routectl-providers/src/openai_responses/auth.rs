...
//!
//! - `ApiKey` (deferred to CG.E): the standard OpenAI surface at
...
pub(crate) fn apply(
    ...
    match cfg.auth_kind {
        ...
        AuthKind::ApiKey => Err(Error::Auth(format!(
            "openai-responses provider `{}`: api-key auth_kind not yet supported (CG.E)",
            cfg.id
        ))),
    ...
    fn api_key_auth_returns_not_implemented() {
        ...
        match err {
            Error::Auth(msg) => {
                ...
                assert!(msg.contains("CG.E"), "msg: {msg}");
            }
