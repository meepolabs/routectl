...
pub async fn chat_completions(
    ...
    if streaming {
        ...
        complete_response(state, req).await
    }
}

async fn complete_response(state: Arc<AppState>, req: ChatRequest) -> Response {
...
async fn stream_response(state: Arc<AppState>, req: ChatRequest) -> Response {
    let stream_result = state.router.stream(req).await;
