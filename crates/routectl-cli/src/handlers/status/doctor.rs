...
/// [`DoctorReport`]'s own `schema_version` (11): the panel embeds that report
/// verbatim, so it must not invent a parallel number.
pub const DOCTOR_SCHEMA_VERSION: u32 = 11;
...
mod tests {
    ...
    fn no_config_path_yields_unavailable_panel() {
        ...
        assert_eq!(panel.schema_version, 11);
    ...
    async fn handler_returns_report_with_no_probe_section() {
        ...
        assert_eq!(json["data"]["report"]["schema_version"], 11);
