...
- `src/schema.rs` -- `requests` table DDL (the capture columns + `request_id UNIQUE` idempotency key + `outcome` CHECK), `meta` table, `SCHEMA_VERSION`; v2 added the nullable `strategy TEXT` column (the per-request auto-cache decision token) via the migrate-on-open ladder (`ALTER TABLE requests ADD COLUMN strategy`, same end shape whether created fresh at v2 or migrated from v1)
