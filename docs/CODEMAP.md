...
- `src/ingress/mod.rs` -- `IngressAdapter` trait, `SseEvent`, `StreamRequestContext` (request-derived seed for `new_stream_state`: local input-token estimate + resolved model), `read_alias_header` (`x-routectl-alias` override)
