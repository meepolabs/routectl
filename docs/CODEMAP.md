...
Native Google Gemini egress (`generateContent` / `streamGenerateContent`, v1beta REST); auth is an API key sent as `x-goog-api-key`. Gated on the `gemini` cargo feature.

- `src/gemini/mod.rs` -- `GeminiProvider` + `GeminiConfig` (fields: `id`, `auth`, `base_url`, `header_extras`, `user_agent`); builds the `models/{model}:generateContent` / `:streamGenerateContent?alt=sse` URLs and the `GEMINI_FORMAT` (`gemini-v1`) reasoning tag; SSE drain
- `src/gemini/auth.rs` -- `apply` injects the `x-goog-api-key` header (no `Authorization`); key resolved per request so a managed token source can rotate
