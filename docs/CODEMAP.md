...
- `src/gemini/mod.rs` -- `GeminiProvider` + `GeminiConfig` (fields: `id`,
  ...
  without copying a literal) and `deprecated_alias_replacement(&str)`;
...
- `src/gemini/cloudcode.rs` -- Cloud Code ("antigravity") egress:
  ...
  mapped to sweep-verified replacements; cloud-code lane only);
  `{project,request,model}` request envelope + `response`-wrapper
...
- `tests/anthropic_reasoning_format_redaction.rs` -- isolated binary pinning
  the foreign-format WARN's `skipped_formats` field under
...
- `tests/documented_provider_blocks.rs` -- docs-vs-schema tripwire: every
  ...
  (`deny_unknown_fields`) instead of at an operator's next startup. A block
...
- `tests/scrub_gate_provider_shapes.rs` -- drift detector over the
  ...
  ; ALSO carries the gitleaks path-allowlist scope block: pins the allowlist
  entry set exactly and asserts the committed driver fixture corpus stays OUT
  of every allowlist surface (singular, plural, and rule-scoped), matched
