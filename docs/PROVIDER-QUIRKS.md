...
The `openai-responses` provider in `chatgpt-oauth` mode is **intentionally a header-bytewise impersonation of the codex CLI**. The chatgpt.com backend gates the Codex / ChatGPT-Pro responses API behind a fingerprint match: the User-Agent literal, the `originator` value, the per-process identity headers, and the OAuth refresh client's own header set must all match what the official codex CLI sends. Drift on any single value risks OpenAI account revocation and a phone-number step-up challenge to recover -- not a recoverable build error, an account-level enforcement event.

**Pinned codex version (impersonation source of truth)**:
...
Both the impersonation worktree and any future maintainer must arrive at the same pin independently from this file. Treat the pin as locked.
...
| `version` / per-request build tag   | passed through `CodexRequestBuilder` per call                 | Matches the codex CLI build version the operator is impersonating.   |
...
The OAuth refresh client (used for `grant_type=refresh_token` POSTs to `https://auth.openai.com/oauth/token`) carries its OWN header set distinct from the responses-API client; both are in scope for the impersonation.
...
**Account-revocation risk**: future deviations (a UA bump on the routectl side that codex did not ship; a missing identity header; a refresh-client header re-ordering visible to TLS fingerprinting) are NOT debuggable as build / wire errors. The first symptom is an OpenAI account in a "needs phone re-auth" lockout state and a refresh endpoint that 401s on every retry. Recovery requires the operator to re-verify their phone number through the ChatGPT web UI. Treat any change here with the same gravity as a database migration on a production system.
