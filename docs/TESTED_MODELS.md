...
covered by the default `[retry]` predicate (every 4xx/5xx is
fallbackable when neither `retry_allowlist` nor `retry_denylist`
is set) so a sibling host can take over without killing the
request.
