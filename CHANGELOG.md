...
- **`routectl login` prints the provider entry that consumes the new seat.** A successful login stored a working credential and wrote nothing to `config.toml`, leaving no seat using it and no sign a second step was needed. The success output now carries a ready-to-paste `[providers.<name>]` block with the right `kind`, the auth selector that kind needs (`auth_kind` / `auth_mode`), and the `api_key_ref` -- including the `#<label>` fragment when `--label` was passed, so a labelled login cannot be wired to the default seat by accident. Nothing is written: the operator pastes the block, so no existing provider entry can be clobbered.
...
- **`ROUTECTL_LOG_REDACT_PROMPTS=1` now also covers unrecognized reasoning format tags.** BREAKING for anyone parsing the field: the foreign-format reasoning WARN's `skipped_formats` previously echoed a caller-chosen `format` tag verbatim even with the redaction knob on, which is the one free-text channel that survived an explicit opt-in. Tags in routectl's recognized vocabulary (`anthropic-claude-v1` plus the Responses-family tags) still echo in both knob states; a tag outside it now renders as the literal `<unrecognized>` under the knob, with every unknown tag in a request sharing that one placeholder slot. Run with the knob off to read the literal of an unrecognized tag. See [docs/LOGGING.md](docs/LOGGING.md) "Redaction".
